...
@property (atomic, copy) NSArray<NSNumber*> *arrayCachedFramesIndex;//save the frame index has cached
...
- (void)autoCacheFrameDuringPlayback:(UIImage *)image atIndex:(NSInteger)index{
    if (!self.isAllFramesLoaded) {
        if ([self.arrayCachedFramesIndex containsObject:[NSNumber numberWithInteger:index]]) {
            return;
        }
        //NSLog(@"will cache a new frame at index : --> %ld",index);
        NSMutableArray<NSNumber*> *cachedFrameIndexs = [NSMutableArray arrayWithArray:self.arrayCachedFramesIndex];
        [cachedFrameIndexs addObject:[NSNumber numberWithInteger:index]];
        self.arrayCachedFramesIndex = cachedFrameIndexs;
        ...
        if (![self.arrayCachedFramesIndex containsObject:[NSNumber numberWithInteger:0]]){
